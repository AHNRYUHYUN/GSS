...
빌딩 단위 게이트웨이에서 수집되는 **도어/각도 센서 데이터**를 수집·보정·저장하고,  
**임계치(Alert)** 및 **리포트(HWPX/CSV)** 생성, **대시보드 실시간 표시**를 통해 **각도 이상치(Anomaly)** 를 모니터링하는 Node.js 기반 서비스입니다.
...
- **날씨**: 주소 → 좌표(Nominatim) → OWM 현재 날씨 저장 → 리포트/대시보드 사용  
- **리포트**: 기간·빌딩 기준 통계(각도/날씨)로 HWPX/CSV 생성 후 응답
- **이미지**: AWS S3를 사용하여 이미지 저장
